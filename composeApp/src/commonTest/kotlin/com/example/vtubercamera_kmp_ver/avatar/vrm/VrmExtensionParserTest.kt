...
class VrmExtensionParserTest {
    ...
    @Test
    fun parsePreviewMetadataExtractsVrm0PreviewFieldsOnly() {
        val glb = createGlb(
            json = """
                {
                  "asset": {"version": "2.0"},
        ...
        val document = VrmExtensionParser.parseDocument(glb).getOrThrow()

        val previewMetadata = VrmExtensionParser.parsePreviewMetadata(document).getOrThrow()

        assertEquals("Preview Only Avatar", previewMetadata.avatarName)
        assertEquals(listOf("OpenAI"), previewMetadata.authors)
        assertEquals("0.95", previewMetadata.vrmVersion)
        assertEquals(1, previewMetadata.thumbnailImageIndex)
    }

    @Test
    fun parsePreviewMetadataExtractsVrm1SpecVersionWhenMetaVersionIsMissing() {
        val glb = createGlb(
            json = """
                {
                  "asset": {"version": "2.0"},
        ...
        assertEquals("VRM 1 Preview", previewMetadata.avatarName)
        assertEquals(listOf("OpenAI", "Codex"), previewMetadata.authors)
        assertEquals("1.0", previewMetadata.vrmVersion)
        assertEquals(0, previewMetadata.thumbnailImageIndex)
    }
